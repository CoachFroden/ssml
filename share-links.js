...
function ensureButton() {
  ...
  ensureStyles();
  ensureDialog();
  let button = tools.querySelector("#share-selected-parts");
  if (!button) {
...
document.addEventListener("click", event => {
  const shareButton = event.target.closest?.("#share-selected-parts");
  if (shareButton) {
  ...
  if (event.target.closest?.(".song-card")) scheduleEnsureButton();
}, true);

document.addEventListener("keydown", event => {
  if (event.key === "Enter" && event.target.closest?.(".song-card")) scheduleEnsureButton();
}, true);

document.addEventListener("change", event => {
  if (event.target.matches?.('#part-list input[type="checkbox"]')) {
    ensureButton();
