...
function makeThumb(canvas,pageNo,part=state.activePart,pdf=state.pdfDoc,pages=state.visiblePages){const thumb=document.createElement("button");thumb.className="page-thumb";thumb.type="button";thumb.append(canvas);thumb.insertAdjacentHTML("beforeend",`<small>Side ${pageNo}</small>`);thumb.addEventListener("click",()=>{state.activePart=part;state.pdfDoc=pdf;state.visiblePages=pages;state.selectedPage=String(pageNo);renderPartList();updatePreviewFocus();openLargePage(pageNo);});return thumb;}
