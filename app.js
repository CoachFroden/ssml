import { initFirebase, isFirebaseConfigured, signIn, signOutUser, observeAuth, fetchSongs, saveSong, replacePartPdf, updateSongParts, updateSongMetadata, deleteSong, analyzeSongPdf, applySongAnalysis } from "./firebase.js?v=11";
...
async function handleImport(event){ event.preventDefault();if(!state.files.length){toast("Vel minst éi PDF-fil.","error");return;}const button=$("#save-song");button.disabled=true;button.textContent=state.demo?"Lagrar …":"Klargjer PDF-ar …";const enteredTitle=$("#song-title").value.trim();const base={title:enteredTitle||state.files[0].name.replace(/\.pdf$/i,""),composer:$("#composer").value.trim(),arranger:$("#arranger").value.trim(),mode:$("input[name=pdf-mode]:checked").value,createdAt:new Date().toISOString()};try{let song;if(state.demo){song={id:`local-${Date.now()}`,...base,parts:state.files.map((file,index)=>({id:`local-part-${Date.now()}-${index}`,name:base.mode==="combined"?"Samla partitur":file.name.replace(/\.pdf$/i,""),fileName:file.name,url:URL.createObjectURL(file),pageCount:null}))};saveLocalMetadata(song);state.songs.unshift(song);finishImportUi();openSong(song.id);toast("Demo-import er lagra utan AI-analyse.");return;}let enhancedFiles=[];if($("#enhance-scans").checked){const results=await enhancePdfFiles(state.files,message=>button.textContent=message);enhancedFiles=results.map(result=>result.enhanced);const improved=results.filter(result=>result.changed).length;if(improved)toast(`${improved} skanna PDF-fil(er) vart forbetra. Originalane blir òg lagra.`);}button.textContent="Lastar opp og analyserer …";song=await saveSong(base,state.files,enhancedFiles);state.pendingSong=song;toast("PDF-en er lasta opp. AI analyserer notane …");const analysis=await analyzeSongPdf(song,null,(current,total)=>button.textContent=`AI analyserer bolk ${current} av ${total} …`);state.pendingAnalysis=analysis;finishImportUi();openAnalysisReview(song,analysis);}catch(error){console.error(error);if(state.pendingSong&&!state.songs.some(x=>x.id===state.pendingSong.id)){state.songs.unshift(state.pendingSong);renderAll();}toast(aiErrorMessage(error),"error");}finally{button.disabled=false;button.textContent="Lagre i arkivet";} }
...
async function saveAppendedPages(event){
  ...
  try{
    ...
    state.activeSong.parts=sortParts(nextParts);state.activePart=state.activeSong.parts.find(item=>item.id===part.id);state.selectedPartIds.add(part.id);previewPdfCache.clear();state.pdfDoc=null;state.viewOriginal=false;
...
async function togglePdfSource(){state.viewOriginal=!state.viewOriginal;state.pdfDoc=null;state.previewVersion++;renderSongDetail();await renderSelectedPreview();toast(state.viewOriginal?"Viser original-PDF-en.":"Viser den forbetra PDF-en.");}

async function getPreviewPdf(part){
  const sourceUrl=partSourceUrl(part);
  if(!sourceUrl)return null;
  if(previewPdfCache.has(sourceUrl))return previewPdfCache.get(sourceUrl);
...
async function printSelectedParts(){const parts=sortParts((state.activeSong.parts||[]).filter(part=>state.selectedPartIds.has(part.id)));if(!parts.length){toast("Vel minst éi stemme.","error");return;}const button=$("#print-selected-parts");button.disabled=true;button.textContent="Klargjer …";const jobTitle=parts.length===1?`${state.activeSong.title} – ${parts[0].name}`:`${state.activeSong.title} – ${parts.length} stemmer`;const win=openPrintWindow(jobTitle);if(!win){button.disabled=false;updatePartSelectionTools();toast("Nettlesaren blokkerte utskriftsvindauget.","error");return;}try{const pdfjs=await import("https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.min.mjs");pdfjs.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.worker.min.mjs";const documents=new Map();const output=[];let completed=0;const total=parts.reduce((sum,part)=>sum+(part.pageNumbers?.length||part.pageCount||1),0);for(const part of parts){const sourceUrl=partSourceUrl(part);if(!sourceUrl)continue;let pdf=documents.get(sourceUrl);if(!pdf){pdf=await pdfjs.getDocument(sourceUrl).promise;documents.set(sourceUrl,pdf);}const pages=part.pageNumbers?.length?part.pageNumbers:Array.from({length:pdf.numPages},(_,i)=>i+1);output.push(`<h1>${escapeHtml(state.activeSong.title)} – ${escapeHtml(part.name)}</h1>`);for(const pageNo of pages){setPrintStatus(win,`Klargjer side ${completed+1} av ${total} …`);const page=await pdf.getPage(pageNo);const viewport=page.getViewport({scale:2});const canvas=document.createElement("canvas");canvas.width=viewport.width;canvas.height=viewport.height;await page.render({canvasContext:canvas.getContext("2d"),viewport}).promise;output.push(`<img alt="${escapeHtml(part.name)}, side ${pageNo}" src="${canvas.toDataURL("image/jpeg",.96)}">`);completed++;}}await finishPrintWindow(win,output.join(""));toast(`${parts.length} stemme(r) er klargjorde for utskrift.`);}catch(error){console.error(error);showPrintError(win);toast("Kunne ikkje klargjere dei valde stemmene.","error");}finally{button.disabled=false;updatePartSelectionTools();}}

async function printPageNumbers(pages,copies=1){const win=openPrintWindow(`${state.activeSong.title} – ${state.activePart.name}`);if(!win){toast("Nettlesaren blokkerte utskriftsvindauget.","error");return;}try{if(!state.pdfDoc)await loadPreview(state.activePart);if(!state.pdfDoc)throw new Error("PDF-en kunne ikkje lastast.");const renderedPages=[];for(let index=0;index<pages.length;index++){const pageNo=pages[index];setPrintStatus(win,`Klargjer side ${index+1} av ${pages.length} …`);const page=await state.pdfDoc.getPage(pageNo);const viewport=page.getViewport({scale:2});const canvas=document.createElement("canvas");canvas.width=viewport.width;canvas.height=viewport.height;await page.render({canvasContext:canvas.getContext("2d"),viewport}).promise;renderedPages.push({pageNo,url:canvas.toDataURL("image/jpeg",.96)});}const images=[];for(let copy=1;copy<=copies;copy++){for(const rendered of renderedPages){images.push(`<img alt="Side ${rendered.pageNo}, eksemplar ${copy}" src="${rendered.url}">`);}}await finishPrintWindow(win,images.join(""));const total=pages.length*copies;toast(`${copies} eksemplar klargjorde (${total} utskriftssider). La talet på kopiar stå på 1 i utskriftsvindauget.`);}catch(error){console.error(error);showPrintError(win);toast("Kunne ikkje klargjere sidene for utskrift.","error");}}
...
function cleanupPrintArea(){if(previousDocumentTitle)document.title=previousDocumentTitle;activePrintOverlay?.remove();activePrintFrame?.remove();activePrintOverlay=null;activePrintFrame=null;activePrintTitle="";previousDocumentTitle="";}
