import { initFirebase, isFirebaseConfigured, signIn, signOutUser, observeAuth, fetchSongs, saveSong, queuePdfEnhancements, addSongPart, replacePartPdf, updateSongParts, updateSongMetadata, deleteSongPart, deleteSong, analyzeSongPdf, analyzeNewInstrumentPdf, applySongAnalysis } from "./firebase.js?v=25";
...
async function handleDeleteActivePart(){const song=state.activeSong,part=state.activePart;if(!song||!part)return;if((song.parts||[]).length<=1){toast("Den siste stemma kan ikkje slettast. Slett heller heile songen.","error");return;}if(!confirm(`Slette stemma «${part.name}» frå «${song.title}»? Dette kan ikkje angrast.`))return;const button=$("#delete-active-part");button.disabled=true;button.textContent="Slettar …";try{state.previewVersion++;clearPreviewPdfCache();let nextParts;if(state.demo){nextParts=(song.parts||[]).filter(item=>item.id!==part.id);const stored=loadLocalSongs().map(item=>item.id===song.id?{...item,parts:nextParts}:item);localStorage.setItem("ssml-demo-songs",JSON.stringify(stored));}else{nextParts=await deleteSongPart(song,part.id);}song.parts=sortParts(nextParts);state.activePart=song.parts[0]||null;state.selectedPartIds=new Set(state.activePart?[state.activePart.id]:[]);state.selectedPage="all";renderSongDetail();if(state.activePart)await loadPreview(state.activePart);renderAll();toast(`Stemma «${part.name}» er sletta.`);}catch(error){console.error(error);button.disabled=false;button.textContent="Slett stemme";toast(error.message||"Kunne ikkje slette stemma.","error");}}
...
function makeThumb(canvas,pageNo,part=state.activePart,pdf=state.pdfDoc,pages=state.visiblePages){const wrap=document.createElement("div");wrap.className="page-thumb-wrap";const thumb=document.createElement("button");thumb.className="page-thumb";thumb.type="button";thumb.append(canvas);const pagePosition=Math.max(0,pages.indexOf(pageNo))+1;const pdfLabel=pageNo!==pagePosition?`<span class="pdf-page-number">PDF-side ${pageNo}</span>`:"";thumb.insertAdjacentHTML("beforeend",`<small>Side ${pagePosition} av ${pages.length}${pdfLabel}</small>`);thumb.addEventListener("click",async()=>{state.activePart=part;try{state.pdfDoc=await getPreviewPdf(part);}catch{state.pdfDoc=pdf;}state.visiblePages=pages;state.selectedPage=String(pageNo);renderPartList();updatePreviewFocus();await openLargePage(pageNo);});const remove=document.createElement("button");remove.className="delete-page-btn";remove.type="button";remove.textContent="Slett side";remove.addEventListener("click",()=>removePageFromPart(part,pageNo,pdf));wrap.append(thumb,remove);return wrap;}
