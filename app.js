...
async function handleImport(event){ event.preventDefault();if(!state.files.length){toast("Vel minst éi PDF-fil.","error");return;}const button=$("#save-song");button.disabled=true;button.textContent=state.demo?"Lagrar …":"Lastar opp originalar …";const enteredTitle=$("#song-title").value.trim();const base={title:enteredTitle||state.files[0].name.replace(/\.pdf$/i,""),composer:$("#composer").value.trim(),arranger:$("#arranger").value.trim(),mode:$("input[name=pdf-mode]:checked").value,createdAt:new Date().toISOString()};const sourceFiles=[...state.files];try{let song;if(state.demo){song={id:`local-${Date.now()}`,...base,parts:sourceFiles.map((file,index)=>({id:`local-part-${Date.now()}-${index}`,name:base.mode==="combined"?"Samla partitur":file.name.replace(/\.pdf$/i,""),fileName:file.name,url:URL.createObjectURL(file),pageCount:null}))};saveLocalMetadata(song);state.songs.unshift(song);finishImportUi();openSong(song.id);toast("Demo-import er lagra utan AI-analyse.");return;}song=await saveSong(base,sourceFiles,[]);state.pendingSong=song;if($("#enhance-scans").checked){button.textContent="Legg PDF-forbetring i kø …";try{song.parts=await queuePdfEnhancements(song);toast("Originalane er lagra. PDF-forbetringa køyrer trygt i bakgrunnen.");}catch(queueError){console.error(queueError);toast("Originalane er lagra, men PDF-forbetringa kunne ikkje leggjast i kø.","error");}}button.textContent="AI analyserer filene …";toast("PDF-ane er lasta opp. AI analyserer notane i kontrollerte bolkar …");const analysis=await analyzeSongPdf(song,sourceFiles);state.pendingAnalysis=analysis;finishImportUi();openAnalysisReview(song,analysis);}catch(error){console.error(error);if(state.pendingSong&&!state.songs.some(x=>x.id===state.pendingSong.id)){state.songs.unshift(state.pendingSong);renderAll();finishImportUi();}toast(aiErrorMessage(error),"error");}finally{button.disabled=false;button.textContent="Lagre i arkivet";} }
